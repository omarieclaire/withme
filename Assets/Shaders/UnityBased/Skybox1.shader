...
Shader "Terrain/Skybox1"
{
  ...
  SubShader{
    ...
    Pass{
      ...
      float _AuroraSpeed;
      float _AuroraHueStart;
      float _AuroraHueSize;
      float _AuroraHorizonImportance;
      float _AuroraNoiseSize;
      ...
      float4 frag (varyings v) : COLOR {
        ...
        for( int i = 0; i < 3; i++ ){
          ...
          // Rotate around the y axis based on noise ?
          fPos = mul( rotationMatrix( float3(0,1,0) , 1* noise( _Time.y * .04 * _AuroraSpeed + fPos * .01 * _AuroraNoiseSize* float3(1,1,1))) , float4(fPos,1)).xyz;


          float v = pow(noise( _AuroraNoiseSize * (fPos + float3(0,_Time.y * 10 * _AuroraSpeed,0)) * .3 * float3(.8,.2 ,1)),2);//sin( fPos.x *100 + _Time.x) * .1 + sin( fPos.y *100 + _Time.y) * .1 + sin( fPos.z *100 + _Time.z) * .1;
          v += noise(_AuroraNoiseSize *fPos * .03+ float3(0,_Time.y * .1* _AuroraSpeed,0)) * 2;
          //v= floor(v* 2) / 2;
          v/=5;
          ...
          tMap =1; 
          //tMap = 1-2 * abs(tmpRD.y);
          /*
          _AuroraIntensity
          _AuroraSpeed
          _AuroraHueStart
          _AuroraHueSize
          _AuroraHorizonImportance
          _AuroraNoiseSize
          */

          float3 hueCol = hsv(_AuroraHueStart + v * _AuroraHueSize + float(i) * _AuroraHueSize * .3,1,1);

          col += hueCol * v * tMap * _AuroraIntensity;
          /*
          if( i == 0 ){
        ...
        col *= pow(saturate(1-_AuroraHorizonImportance*abs(rd.y + noise( v.ro- float3(0,_Time.y * .12,0) )- noise(v.ro * 2 + float3(0,_Time.y * .1,0)))),1);
        col = lerp( col , col * length(col) * length(col) * 3, _AuroraVibrantness);


        /// RAYS
        float3 fLightDir = normalize(_LightDir);
        for( int i = 0; i < 3; i++ ){

          
          float3 hueCol = hsv(_SunHue + float(i) * _SunHueSize,1,1);
          float3 fPos = _WorldSpaceCameraPos * .1  + v.ro * 100+ rd * i * 30.1f;
          col += .5*_SunIntensity * hueCol* pow( noise(fPos * .1),2)*10*pow(  saturate(dot( fLightDir, -normalize(rd))),101);
          col += .2*_SunIntensity * hueCol* pow( noise(fPos * .4),2)*10*pow(  saturate(dot( fLightDir, -normalize(rd))),101);

          
          float2  xy = GetXYCoordsInPlane(_WorldSpaceCameraPos * .1+ v.ro * 400+ rd * i * 100.1f, fLightDir ,float3(0,1,0));
