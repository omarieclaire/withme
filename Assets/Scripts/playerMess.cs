...
//     public float kaliTransitionDuration = 3f;  // Time between color transitions

//     private Color kaliTargetColor;  // The color we're transitioning to
//     private Color kaliCurrentColor;  // The current color of the trail
//     private float kaliTransitionTimer = 0f;  // Timer to track transition progress
//     private float kaliCurrentHue;  // Keep track of the current hue
//     private TrailRenderer kaliTrailRenderer;  // This player's trail renderer

//     public float kaliChangeInterval = 3f;

//     private float kaliTimer;
...
//         kaliTrailRenderer = GetComponent<TrailRenderer>();

//         // Start with a random hue and log it
//         kaliCurrentHue = Random.value;
//         Debug.Log($"[DEBUG] Starting Hue: {kaliCurrentHue}");

//         kaliCurrentColor = Color.HSVToRGB(kaliCurrentHue, colorSaturation, colorValue);
//         Debug.Log($"[DEBUG] Initial Color: {kaliCurrentColor}");

//         kaliTargetColor = KaliGetNextColor(kaliCurrentHue);
//         Debug.Log($"[DEBUG] Initial Target Color: {kaliTargetColor}");

//         KaliUpdateTrailRendererColor();
//     }
...
//         if (SceneManager.GetActiveScene().name == "Kali")
//         {

//             kaliTimer += Time.deltaTime;
//             if (kaliTimer >= kaliChangeInterval)
//             {
//                 regularRing.material.color = new Color(Random.value, Random.value, Random.value);
//                 kaliTimer = 0f;
//             }


//             kaliTransitionTimer += Time.deltaTime;
//             Debug.Log($"[DEBUG] Transition Timer: {kaliTransitionTimer}");

//             if (kaliTransitionTimer > kaliTransitionDuration)
//             {
//                 kaliTransitionTimer = 0f;  // Reset the timer

//                 // Increment the hue and log the updated hue
//                 kaliCurrentHue = Mathf.Repeat(kaliCurrentHue + 0.1f, 1f);
//                 Debug.Log($"[DEBUG] yo Updated Hue: {kaliCurrentHue}");

//                 // Get the next color and log it
//                 kaliTargetColor = KaliGetNextColor(kaliCurrentHue);
//                 Debug.Log($"[DEBUG] yo New Target Color: {kaliTargetColor}");
//             }

//             // Smoothly interpolate between the current color and the target color
//             kaliCurrentColor = Color.Lerp(kaliCurrentColor, kaliTargetColor, kaliTransitionTimer / kaliTransitionDuration);
//             Debug.Log($"[DEBUG] yo Interpolated Current Color: {kaliCurrentColor}");
...
//              if (regularRing != null)
//             {
//                 regularRing.material.color = kaliTargetColor;
//                 // Debug.Log($"[DEBUG] Player ID: {id} | Regular Ring Color: {regularRing.material.color}");
//             }
//             if (chargedRing != null)
//             {
//                 chargedRing.material.color = Color.Lerp(kaliTargetColor, Color.white, 0.5f);  // Glow effect
//                                                                                     // Debug.Log($"[DEBUG] Player ID: {id} | Charged Ring Color: {chargedRing.material.color}");
//             }
//             if (maxRing != null)
//             {
//                 float pulse = Mathf.PingPong(Time.time * 2f, 1f);  // Pulsing effect for max ring
//                 maxRing.material.color = Color.Lerp(kaliTargetColor, Color.yellow, pulse);  // Make max ring flash yellow
//                                                                                   // Debug.Log($"[DEBUG] Player ID: {id} | Max Ring Color: {maxRing.material.color}");
//             }
//             if (text != null)
//             {
//                 text.color = kaliTargetColor;
//                 // Debug.Log($"[DEBUG] Player ID: {id} | Text Color: {text.color}");
//             }

//             // color = kaliTargetColor;
...
//     private void KaliUpdateTrailRendererColor()
//     {
//         if (kaliTrailRenderer != null)
//         {
//             kaliTrailRenderer.startColor = kaliCurrentColor;
//             kaliTrailRenderer.endColor = kaliCurrentColor;
//             Debug.Log($"[DEBUG] TrailRenderer Color Applied: {kaliCurrentColor}");
//         }
//         else
//         {
...
//     private Color KaliGetNextColor(float hue)
//     {
