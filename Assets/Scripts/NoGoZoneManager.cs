...
public class NoGoZoneManager : MonoBehaviour
{
    ...
    public Collider floorCollider; // New floor collider
    ...
    // Method to check no-go zones and prevent going below the floor
    public bool IsInNoGoZone(Vector3 position)
    {
        ...
        // Check if the position is beneath the floor
        if (floorCollider != null && position.y < floorCollider.bounds.max.y)
        {
            // Debug.Log("Position is below the floor.");
            return true;
        }
